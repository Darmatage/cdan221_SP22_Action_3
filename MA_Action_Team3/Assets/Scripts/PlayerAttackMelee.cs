...
public class PlayerAttackMelee : MonoBehaviour{
      ...
      void Attack(){
			...
			Collider2D[] hitChests = Physics2D.OverlapCircleAll(attackPt.position, attackRange, chestLayers);
           
            foreach(Collider2D chest in hitChests){
                  Debug.Log("We hit " + chest.name);
                  chest.GetComponent<TreasureChest_Damage>().TakeDamage(attackDamage);
            }
